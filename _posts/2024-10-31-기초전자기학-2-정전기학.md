...
> $\displaystyle \oint \vec{E} \cdot d \vec{a} = E \cdot 2\pi s \cdot L \implies Q_{inside} = \int dv \rho = \int_{0}^S ds' s' \int_{0}^{2\pi} d \phi' \int_{0}^L dz' ks' = 2\pi L \cdot \frac{% raw %}{{ks^3}}{% endraw %}{3}$
