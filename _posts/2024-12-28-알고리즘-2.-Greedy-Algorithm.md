...
> 목표는, 복잡한 그래프에서 올바른 Minimum Spanning Tree를 추출하는 것이다. 정답이라고 이미 밝혀진 Minimum Spanning Tree를 $T_{mst}$라고 가정하자. Prim Algorithm을 이용해서 내가 만들고있는 Minimum Spanning Tree를 $T$라고 가정하자. $T$와 $T_{mst}$는 Edge Set이다.
{: .prompt-tip}
> 
