...
tags: ["포사체 운동", "미분 방정식", "공기 저항", "경로", "사정거리", "지구 회전 효과", "초기 속도", "좌표계"]
math: true
toc: true
comments: true
---
...
$$
\ddot{ \mathbf{r}}=-g \mathbf{k} \implies\dot{ \mathbf{r} } = -gt \mathbf{k} + \mathbf{v}_{0} =>\displaystyle \mathbf{r} = -\frac{1}{2} gt^2 \mathbf{k}+ \mathbf{v_{0}}t+\mathbf{r_{0}}
$$
...
- 공간 상의 경로 : $$x-z$$의 관계
	- x는 $$x=v_{0}\cos \alpha t$$이므로 $$\displaystyle t=\frac{x}{v_{0}\cos \alpha}$$를 $$z(t)$$에 넣으면 다음과 같다.
...
$$\dot{x}$$, $$\dot{y}$$는 바로 $$\dot{x}=\dot{x_{0}}e^{-\gamma t}$$, $$\dot{y}=\dot{y_{0}}e^{-\gamma t}$$ 꼴임을 알 수 있다. $$\dot{z}$$는 $$\displaystyle \ddot{z}=-\gamma \left( \dot{z}-\frac{g}{\gamma} \right)$$ 꼴로 쓰고 $$\displaystyle \dot{z}-\frac{g}{\gamma}=p$$로 치환하면 구할 수 있다. 
