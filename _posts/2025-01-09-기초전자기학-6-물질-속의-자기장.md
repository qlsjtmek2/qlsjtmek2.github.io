...
tags: ["전자기학", "자기장", "자성체", "쌍극자", "토크", "전기장", "자화", "자기장"]
math: true
toc: true
comments: true
---
...
이 결과를 대입하면 $$\displaystyle I \oint_{C} d\vec{l} \times (\cancel{ \vec{B}(\vec{r}_{0}) } + ((\vec{r} - \cancel{ \vec{r}_{0} })  \cdot \nabla) \vec{B}(\vec{r})|_{\vec{r}=\vec{r}_{0}})$$이다. 같은 논리로 $$\vec{B}(\vec{r}_{0})$$과 $$\vec{r}_{0}$$ 항은 적분범위에 포함되지 않아 밖으로 빠지면 적분 결과가 0이다. 
...
$$\displaystyle F_{i} = I \sum_{j,k} \epsilon_{ijk} \oint_{C} dl_{j} (\vec{r} \cdot \nabla)B_{k}(\vec{r})|_{\vec{r}=\vec{r}_{0}} = I \sum_{j,k} \epsilon_{ijk}\oint_{C} dl_{j} \sum_{l}x_{l}\partial_{l}B_{k}(\vec{r})|_{\vec{r}=\vec{r}_{0}}$$이고, $$\partial_{l}B_{k}(\vec{r})|_{\vec{r}=\vec{r}_{0}}$$은 최종적으로 어떤 상수벡터가 될 것이므로, $$\vec{c}$$로 치환하여 생각한다. 
