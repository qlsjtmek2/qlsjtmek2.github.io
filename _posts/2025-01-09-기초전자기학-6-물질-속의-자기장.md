...
tags: ["전자기학", "자기장", "자성체", "쌍극자", "힘과 토크", "전기장", "자화", "자기장"]
math: true
toc: true
comments: true
---
...
$$\displaystyle F_{i} = I \sum_{j,k} \epsilon_{ijk} \oint_{C} dl_{j} (\vec{r} \cdot \nabla)B_{k}(\vec{r})\mid_{\vec{r}=\vec{r}_{0}} = I \sum_{j,k} \epsilon_{ijk}\oint_{C} dl_{j} \sum_{l}x_{l}\partial_{l}B_{k}(\vec{r})\mid_{\vec{r}=\vec{r}_{0}}$$이고, $$\partial_{l}B_{k}(\vec{r})|_{\vec{r}=\vec{r}_{0}}$$은 최종적으로 어떤 상수벡터가 될 것이므로, $$\vec{c}$$로 치환하여 생각한다.
