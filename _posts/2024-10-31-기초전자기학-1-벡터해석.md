...
tags: ["전자기학", "델 연산자", "스칼라 필드", "벡터 필드", "디랙 델타 함수", "미분 규칙", "발산 정리", "함수 적분"]
math: true
toc: true
comments: true
---
...
> 
> $$
> \oint\_{S} \frac{1}{r^2} \hat{r} \cdot d\vec{a} = \oint\_{S} \left( \frac{1}{R^2} \hat{r} \right) \cdot  (R^2 \sin \theta d\theta d\phi \hat{r})= \int^\pi\_{0} \int\_{0}^{2\pi} \sin \theta d\theta d\phi= \left( \int\_{0}^{\pi} \sin \theta d\theta \right) \left( \int\_{0}^{2\pi} d\phi \right)
> $$
> 
> 
> $$
