...
open path는 시작과 끝점이 있는 path, close path는 경로가 닫힌 path를 의미한다. 선적분을 하기 위해 $[a,b]\to C$로 가는 변환 함수 $\vec{r} = \vec{r}(t) = (x(t),y(t),z(t))$라는걸 반드시 찾아야 함. 만약 곡선 $C$가 $x=y^2$ 위의 곡선이라면 $\vec{r}(t)=(t^2,t)$어야 저 조건을 만족하므로 이런 매개변수방정식을 만들어 공간을 변환할 수 있다. 만약 찾기 무지 어려우면 Spline으로 근사 곡선을 찾을 수도 있겠다.
$\displaystyle \int_{C} \vec{A}(\vec{r}) \cdot d\vec{r} = \int^{t_{1}}\_{t_{0}} \vec{A} (\vec{r}(t)) \cdot \frac{d\vec{r}}{dt} dt = \int^{t_{1}}\_{t_{0}} \vec{A}(\vec{r}(t)) \cdot\vec{r}'(t) dt$
만약 $\vec{A}(x,y)=(A_{x}(x,y), A_{y}(x,y))$라면 실제로 점곱과 같이 연산할 수 있다. $\displaystyle \int_{C} \vec{A}(x,y) \cdot d\vec{r} = \int_{C} A_{x}dx + A_{y}dy$

![Pasted image 20241129111051.png](/assets/img/posts/Pasted image 20241129111051.png){: width="300"}

선적분의 경로가 헷갈린다면, $C$의 경로를 평범한 경로로 바꾼 경로 즉 $-C$를 계산하고 원래대로 돌렸을 때 부호가 어떻게 붙는지 생각해보면 된다. 예로, 위 그림의 경로를 선적분하는데 부호와 적분 범위가 헷갈린다면, 바로 C를 적분하지 말고 $-C$를 적분해서 -를 붙여보라. $\displaystyle \int_{C} \vec{F} \cdot d\vec{r} = -\int_{-C}\vec{F} \cdot d\vec{r} = -\int_{0}^2 (F_{x}, F_{y}) \cdot (dx, 0) = -\left[ \int_{0}^2 F_{x}dx \right]\_{y=1}$
...
Gradiant $f$ = $\nabla f = \displaystyle (\frac{% raw %}{{\partial f}}{% endraw %}{\partial x}, \frac{\partial f}{\partial y}, \frac{\partial f}{\partial z})$. 스칼라 함수 $f$를 넣으면 그 지점에서 함수값이 가장 가파르게 증가하거나 감소하는 방향을 가리키는 기울기 벡터를 반환한다.
...
Curl이란, Vector Field의 회전량을 감지하는 도구다. 어떤 Vector Field에 Curl을 취한다는 것은, 어떤 지점에서 $\vec{A}$가 얼마나 회전하는지를 나타내는 벡터를 반환하는 함수를 만들어준다. 회전의 방향은 오른손 법칙으로 결정되며, 소용돌이의 세기는 벡터의 세기와 같다. 계산은 다음과 같이 가능하다. $\displaystyle \nabla \times \vec{A}= \begin{vmatrix} \mathbf{i} & \mathbf{j} & \mathbf{k} \\\ \frac{\partial}{\partial x} & \frac{\partial}{\partial y} & \frac{\partial}{\partial z} \\\ A_x & A_y & A_z \end{vmatrix}$
...
> 
> $\displaystyle \oint_{\Box} \mathbf{F} \cdot d\mathbf{r} = \int_{\mathrm{I}} + \int_{\mathrm{II}} + \int_{\mathrm{III}} + \int_{\mathrm{IV}}$
> $\displaystyle \int_{\mathrm{I}} + \int_{\mathrm{III}} = \int_x^{x+d x} (F_x(x', y) \, - F_{x}(x', y+dy))dx' = - \int_x^{x+d x} \frac{\partial F_x}{\partial y} \, dy \, dx' = -\frac{\partial F_x}{\partial y} \, dx \, dy$
> $\displaystyle \int_{\mathrm{II}} + \int_{\mathrm{IV}} = \int_y^{y+'\Delta' y} \big(F_y(x+d x, y') - F_y(x, y')\big) \, dy'= \int_y^{y+d y} \frac{\partial F_y}{\partial x} \, dx \, dy' = \frac{% raw %}{{\partial F_{y}}{% endraw %}}{\partial x}dxdy$
> 위 과정에서 '[구간이 아주 작은 적분은, 그냥 dx를 곱한것과 같다.](https://qlsjtmek2.github.io/구간이-아주-작은-적분은,-그냥-dx를-곱한것과-같다.)' 를 사용한다. 따라서 $\displaystyle \oint_{\Box} \mathbf{F} \cdot d\mathbf{r} = \left(\frac{\partial F_y}{\partial x} - \frac{\partial F_x}{\partial y}\right) \, dx \, dy$ 결론을 얻는다.
> 
...
> $\displaystyle \sum_{i} \oint_{C_i} \mathbf{F} \cdot d\mathbf{r} = \sum_{i} \left( \frac{\partial F_y(x_{i},y_{i})}{\partial x} - \frac{\partial F_x(x_{i},y_{i})}{\partial y} \right) dx_i \, dy_i$
> 이때, $dx_{i}dy_{i}=dA_{i}$이므로 최종적으로 $\therefore ~ \displaystyle \oint_{C} \mathbf{F} \cdot d\mathbf{r} = \iint_{S} \left( \frac{\partial F_y}{\partial x} - \frac{\partial F_x}{\partial y} \right) \, dA$ 를 얻는다.
> 
> 미소 사각형의 선적분 값을 구하는 과정을 똑같이 y-z, z-x 평면에 대해 적용할 수 있다. x-z 평면이 아닌 z-x 평면인 이유는, 오른손 법칙에 의해 y가 + 방향이 되도록 순서를 적용한 것이다.
> 
> $\displaystyle \oint_{\Box} \mathbf{F} \cdot d\mathbf{r} = \int \left( \frac{\partial F_z}{\partial y} - \frac{\partial F_y}{\partial z} \right) \, dy \, dz$,  $\displaystyle \oint_{\Box} \mathbf{F} \cdot d\mathbf{r} = \int \left( \frac{\partial F_x}{\partial z} - \frac{\partial F_z}{\partial x} \right) \, dz \, dx$
> 
...
> [!note]- 연관성 증명{title}
> 1. **1->2 증명**
> 임의의 기준점 O에 대하여 함수 f를 $\displaystyle f(\vec{r}) = \int^{\vec{r}}\_{O} \vec{A}(\vec{r}) \cdot d\vec{r}$로 정의하자. $\displaystyle\oint_{C} \vec{A} \cdot d\vec{r} = 0 \implies \int^{\vec{b}}\_{\vec{a}} \vec{A} \cdot d\vec{r} = f(\vec{b})-f(\vec{a})$로 쓸 수 있다. $\displaystyle f(\vec{b})-f(\vec{a})=\int^{\vec{b}}\_{\vec{a}}df$이고, $df=\nabla f \cdot d\vec{r}$이므로. $\displaystyle\int^{\vec{b}}\_{\vec{a}} \vec{A} \cdot d\vec{r} = f(\vec{b})-f(\vec{a}) = \int^{\vec{b}}\_{\vec{a}} \nabla f \cdot d\vec{r} \implies \vec{A} = \nabla f$
> 
> 2. **1->3 증명**
> 1->2 증명에 의해 $\vec{A}$가 Conservative Vector Field이면 $\vec{A} = \nabla f$, $\nabla \times \vec{A} = \nabla \times (\nabla f)$이고, 성분별로 보면 $\displaystyle (\nabla \times (\nabla f))\_{i}= \sum_{j,k} \epsilon_{ijk} \partial_{j} (\nabla f)\_{k} = \sum_{j,k} \epsilon_{ijk} \partial_{j}\partial_{k}f$이고 이걸 j, k에 대해 sum해보면 모두 상쇄되어 $(\nabla \times (\nabla f))\_{i}=0$이다. 따라서 $\nabla \times \vec{A} = 0$이다.
> 
> 3. 2->1 증명
...
$\displaystyle \therefore ~ \iint_{\partial V} \vec{A} \cdot d \vec{s} = \int_{V} \nabla \cdot \vec{A} dV$
...
> 
> $\vec{r}$ 위치를 중심으로 감싸는 미소 체적 V를 생각해보자. $\displaystyle \oint_{\partial V} \vec{A} \cdot d\vec{s}$는 각각 yz, zx, xy 평면에 평행한 두 면의 Flux와 같다. $= \Phi_{yz} + \Phi_{zx} + \Phi_{xy}$
> 
> $\displaystyle\Phi_{yz} = \iint \vec{A} \cdot (dydz \hat{x}) \mid_{x'=x+\frac{dx}{2}} + \iint \vec{A} \cdot (-dydz \hat{x})\mid_{x' = x - \frac{dx}{2}}$
> $\displaystyle = \iint A_{x}\left( x + \frac{dx}{2}, y', z' \right) - A_{x}\left( x-\frac{dx}{2},y',z' \right) dy'dz'$와 같다.
> 
> $f(x,y,z)$를 테일러 전개하면 $\displaystyle f(x,y,z) = f(x_{0},y_{0},z_{0})+(d\vec{r} \cdot \nabla)f(\vec{r})\mid_{\vec{r}=\vec{r}\_{0}}+\frac{1}{2!}\dots$ 이므로 $\displaystyle f\left( x+\frac{dx}{2},y,z \right) = f(\vec{r}\_{0}) + (\frac{dx}{2} \frac{\partial}{\partial x}) f(\vec{r})\mid_{\vec{r}=\vec{r}\_{0}}+ \dots$, $\displaystyle f\left( x-\frac{dx}{2},y,z \right) = f(\vec{r}\_{0}) + (-\frac{dx}{2} \frac{\partial}{\partial x}) f(\vec{r})\mid_{\vec{r}=\vec{r}\_{0}}+ \dots$
> $\displaystyle f\left( x+\frac{dx}{2},y,z \right)-f\left( x-\frac{dx}{2},y,z \right) \simeq \left( dx \frac{\partial}{\partial x} \right)f(\vec{r})\mid_{\vec{r}=\vec{r}\_{0}}$와 같다.
> 
> $\displaystyle \Phi_{yz} = \int^{z+\frac{dz}{2}}\_{z-\frac{dz}{2}} \int^{y+\frac{dy}{2}}\_{y-\frac{dy}{2}} \frac{% raw %}{{\partial A_{x}(x,y',z')}}{% endraw %}{\partial x} dx dy'dz'$
> 
> $\displaystyle \int^{x+\frac{dx}{2}}\_{x-\frac{dx}{2}} f(x)dx = F\left( x+\frac{dx}{2} \right) - F\left( x-\frac{dx}{2} \right) = dx \frac{\partial}{\partial x}F(x) = f(x)dx$이므로,
> $\displaystyle \Phi_{yz} = \frac{\partial A_{x}}{\partial x}dxdydz$와 같다. 이를 zx, xy 평면에도 똑같이 적용 가능하므로..
> 
> $\displaystyle \therefore ~ \iint_{\partial V} \vec{A} \cdot d\vec{s} = \left( \frac{% raw %}{{\partial A_{x}}{% endraw %}}{\partial x} + \frac{% raw %}{{\partial A_{y}}{% endraw %}}{\partial y} + \frac{% raw %}{{\partial A_{z}}{% endraw %}}{\partial z} \right)dxdydz = (\nabla \cdot \vec{A})dV$가 성립한다.
> 
> ![Pasted image 20241211130149.png](/assets/img/posts/Pasted image 20241211130149.png){: width="200"}
> 
> 임의의 체적 $V$에 대한 체적 적분은 미소 체적 $V_{i}$를 계산해서 모두 더하여 계산할 수 있다.
> 
> $\displaystyle \iint_{\partial V_{i}} \vec{A} \cdot d\vec{s}\_{i} = (\nabla \cdot \vec{A})dV_{i} \implies \sum_{i} \iint_{\partial V_{i}} \vec{A} \cdot d\vec{s}\_{i} = \sum_{i}(\nabla \cdot \vec{A}) dV_{i}$
> 
...
$\displaystyle=  \partial_1 \partial_2 - \partial_2 \partial_1 + \partial_2 \partial_3 - \partial_3 \partial_2 +\partial_3 \partial_1 - \partial_1 \partial_3 = 0$
